...
Reconfig(i, newConfig, newVersion) ==
    ...
    /\ (1 = Cardinality(((newConfig \ config[i]) \cup (config[i] \ newConfig)))) \* gt: QuorumsOverlap(config[i], newConfig)
    /\ OplogCommitment(i) \* gt: OplogCommitment(i)
    /\ i \in newConfig \* gt: i \in newConfig
    /\ newConfig # config[i] \* gt: newConfig # config[i]
    /\ newVersion = configVersion[i] + 1 \* gt: newVersion = configVersion[i] + 1
    /\ configTerm' = [configTerm EXCEPT ![i] = currentTerm[i]] \* gt: [configTerm EXCEPT ![i] = currentTerm[i]]
    ...
    /\ log' = log \* gt: log
    /\ immediatelyCommitted' = immediatelyCommitted \* gt: immediatelyCommitted
...
BecomeLeaderAction == (\E s \in Server : \E Q \in SUBSET Server : \E newTerm \in Term :  
    BecomeLeader(s, Q, newTerm))
CommitEntryAction == (\E s \in Server : \E Q \in SUBSET Server : CommitEntry(s, Q))
UpdateTermsAction == (\E s,t \in Server : UpdateTerms(s, t))
ReconfigAction == (\E s \in Server, newConfig \in AllConfigs, newVersion \in Version: 
    Reconfig(s, newConfig, newVersion))
