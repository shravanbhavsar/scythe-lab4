...
;;                 /\ ?log#prime = log
;;                 /\ ?immediatelyCommitted#prime = immediatelyCommitted 
;; PROVE  ?configTerm#prime \in [Server -> Nat]
;; TLA+ Proof Manager 1.4.5 (build 33809)
;; Proof obligation #121
;;   generated from file "./mldr_IndProofs.tla", line 236, characters 15-16
...
(assert (not
  (and
    ...
    (= (tla__DOMAIN a_configTermhash_primea) Server)
    (forall ((a_v23a u))
      (=>
        (tla__in a_v23a Server)
        (and
          (tla__in (tla__fcnapp a_configTermhash_primea a_v23a) tla__Int)
          (tla__le (int2u 0) (tla__fcnapp a_configTermhash_primea a_v23a))))))))
(assert
  (forall ((a_v27a u))
    (! (=
