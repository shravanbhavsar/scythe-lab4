---- MODULE mldr3_IndProofs ----
\* configindex: 708
EXTENDS mldr3, FiniteSets, FiniteSetTheorems
...
\* Easy lemma to prove i.e. if symmetric difference between c1 and c2 is <=1, and then quorums overlap between c1 and c2.
\* This is proved, for example, in the Raft paper.
LEMMA OneSymmDiffImpliesOverlap == 
    \A c1,c2 \in SUBSET Server : 
        /\ (
         (1 = Cardinality(((c1 \ c2) \cup (c2 \ c1))))) => QuorumsOverlap(c1, c2)
        /\ Cardinality(SymmDiff(c1, c2)) <= 1 => QuorumsOverlap(c1, c2)
USE OneSymmDiffImpliesOverlap
...
PROOF
   <1>1. QED 
     <2>1. (currentTerm \in [Server -> Nat])'
              BY DEF TypeOK,Init,Next,Safety,
...
PROVE \A s \in Server : NewerOrEqualConfig(CV(s)', CV(s))
    <1>ok. TypeOK BY DEF IndAuto
    <1>1. CASE \E s \in Server, newConfig \in SUBSET Server, nv \in Version : Reconfig(s, newConfig, nv)
        BY <1>ok, <1>1, ReconfigImpliesNewerOrEqualConfig
    <1>2. CASE \E s,t \in Server : \E newTerm \in Term : \E  newVersion \in Version: SendConfig(s, t, newTerm, newVersion)
        BY <1>ok, <1>2, SendConfigImpliesNewerOrEqualConfig
    <1>3. CASE \E s \in Server : \E Q \in SUBSET Server : \E nv \in Term : BecomeLeader(s, Q, nv) /\ BecomeLeader(s, Q, nv)
        <3>1. PICK s \in Server : \E Q \in SUBSET Server, nv \in Term : BecomeLeader(s, Q, nv) /\ BecomeLeader(s, Q, nv) BY <1>3
        <3>2. \A t \in Server : t # s => CV(t)' = CV(t) BY <3>1 DEF BecomeLeader, CV, TypeOK
        <3>3. NewerConfig(CV(s)', CV(s))
            <4>1. currentTerm[s] >= configTerm[s] BY <3>1, ElectedLeadersCurrentTermGreaterThanConfigTerms DEF BecomeLeader
    ...
    <1>4. CASE \E s,t \in Server : \E nv \in Term : UpdateTerms(s,t, nv) /\ UpdateTerms(s,t, nv)
...
ASSUME IndAuto, Next,
       NEW p \in Server,
       NEW nv \in Term,
       \E Q \in Quorums(config[p]) : BecomeLeader(p, Q, nv)
...
PROOF BY QuorumsIdentical DEF Reconfig, ConfigIsCommitted,
            ActiveConfigSet, ConfigDisabled, NewerConfig, CV, Quorums, TypeOK, QuorumsAt, TermQuorumCheck, ConfigQuorumCheck
...
PROOF
    ...
    <1>3. PICK Q \in Quorums(config[s]) : \A n \in Q : currentTerm[n] = currentTerm[s]
        BY QuorumsIdentical DEF Reconfig, ConfigIsCommitted, QuorumsAt, TermQuorumCheck, ConfigQuorumCheck
 ...
 PROVE \A n \in ActiveConfigSet' : n \in ActiveConfigSet
     ...
     <4>. CASE n = p
         <5>1. \E nQ \in Quorums(config[n]) : \A q \in nQ : CV(n) = CV(q)
             BY <4>ok, QuorumsIdentical DEF Reconfig, ConfigIsCommitted, Quorums, CV, QuorumsAt,  TermQuorumCheck, ConfigQuorumCheck
...
DEF Reconfig, IndAuto ,Inv2973_1_1_def,Inv1758_1_0_def,Inv866_1_0_def
...
PROOF
    ...
    <4>2. PICK pQ \in Quorums(config[p]) : \A q \in pQ : CV(q) = CV(p)
        BY <4>ok, QuorumsIdentical DEF Reconfig, ConfigIsCommitted, CV, TypeOK, QuorumsAt,  TermQuorumCheck, ConfigQuorumCheck
...
InitTermType == TRUE
 ...
 THEOREM Init => IndAuto
   <1> SUFFICES ASSUME Init
                PROVE  IndAuto
     OBVIOUS
   <1> USE ServerIsNonempty, QuorumsExistForNonEmptySets DEF OnePrimaryPerTerm, ConfigDisabled, NewerConfig, CV, IsNewerConfig
   <1>0. TypeOK
     BY  ServerIsNonempty, QuorumsExistForNonEmptySets, QuorumsAreNonEmpty, PrimaryAndSecondaryAreDifferent
   ...
   <1>5. Inv3915_1_1_def
     ...
     DEF OnePrimaryPerTerm, ConfigDisabled, NewerConfig, CV, IsNewerConfig,TypeOK,Init,Next,IndAuto,Safety,
         Inv2973_1_1_def,Inv1758_1_0_def,Inv3915_1_1_def,Inv3246_1_2_def,Inv866_1_0_def
   <1>6. Inv3246_1_2_def
     BY ServerIsNonempty, ServerIsFinite, QuorumsExistForNonEmptySets, QuorumsAreNonEmpty, PrimaryAndSecondaryAreDifferent
     DEF OnePrimaryPerTerm, ConfigDisabled, NewerConfig, CV, IsNewerConfig,TypeOK,Init,Next,IndAuto,Safety,
         Inv2973_1_1_def,Inv1758_1_0_def,Inv3915_1_1_def,Inv3246_1_2_def,Inv866_1_0_def,
         Quorums
   <1>7. Inv866_1_0_def
...
THEOREM IndAuto /\ Next => IndAuto'
  <1> SUFFICES ASSUME IndAuto /\ Next
               PROVE  IndAuto'
    OBVIOUS
  <1> USE ServerIsNonempty, QuorumsExistForNonEmptySets, QuorumsAreNonEmpty, PrimaryAndSecondaryAreDifferent
  ...
  <1>1. Safety'
    ...
    <2>3. CASE BecomeLeaderAction
        ...
        <3>5. SUFFICES ASSUME state'[t] = Primary, currentTerm'[s] = currentTerm'[t], s # t
              PROVE FALSE BY <3>3
        <3>6. \/ \E Q \in Quorums(config[s]), nts \in Term : BecomeLeader(s, Q, nts)
              \/ \E Q \in Quorums(config[t]), ntt \in Term : BecomeLeader(t, Q, ntt)
            <4>a. SUFFICES ASSUME /\ ~\E Q \in Quorums(config[s]), nts \in Term : BecomeLeader(s, Q, nts)
                                  /\ ~\E Q \in Quorums(config[t]), ntt \in Term : BecomeLeader(t, Q, ntt)
                  PROVE FALSE OBVIOUS
            <4>p. PICK p \in Server : \E Q \in Quorums(config[p]), nts \in Term : BecomeLeader(p, Q, nts) 
                BY <2>3 DEF BecomeLeader
            <4>q. PICK Q \in Quorums(config[p]), nts \in Term : BecomeLeader(p, Q, nts) /\ BecomeLeader(p, Q, nts) BY <4>p
        ...
        <3>. CASE \E Q \in SUBSET Server, nts \in Term : BecomeLeader(s, Q, nts)
            <4>1. PICK Q \in SUBSET Server, nts \in Term : BecomeLeader(s, Q, nts) BY QuorumsAreServerSubsets
            ...
            <4>4. s \in ActiveConfigSet BY <3>2, <4>1, ElectedLeadersInActiveConfigSet DEF IndAuto, BecomeLeader
            <4>5. \E n \in Q : currentTerm[n] >= configTerm[t] 
                BY <3>2, <4>1, <4>4 
                DEF IndAuto,ActiveConfigSet,Inv3246_1_2_def, BecomeLeader
        ...
        <3>. CASE \E Q \in SUBSET Server, nt \in Term : BecomeLeader(t, Q, nt)
            <4>1. PICK Q \in SUBSET Server, nt \in Term : BecomeLeader(t, Q, nt) OBVIOUS
            ...
            <4>3. currentTerm[s] = configTerm[s]
                ...
                <5>. QED BY <5>2 DEF IndAuto, Inv866_1_0_def
            <4>4. t \in ActiveConfigSet BY <3>2, <4>1, ElectedLeadersInActiveConfigSet DEF IndAuto, BecomeLeader
            <4>5. \E n \in Q : currentTerm[n] >= configTerm[s] BY <3>2, <4>1, <4>4 DEF IndAuto, ActiveConfigSet,Inv3246_1_2_def, BecomeLeader
            <4>6. \E n \in Q : currentTerm[n] > currentTerm[t] BY <2>ok, <3>2, <4>1, <4>2, <4>3, <4>5 DEF Quorums, TypeOK
            <4>. QED BY <2>ok, <3>2, <4>1, <4>6 DEF BecomeLeader, CanVoteForConfig, Quorums, TypeOK
        <3>. QED BY QuorumsAreServerSubsets,<2>3, <3>6 DEF BecomeLeader
  ...
  <1>3. Inv2973_1_1_def'
    <2>ok. TypeOK BY DEF IndAuto
    <2>1. CASE ReconfigAction
        <3>p. PICK p \in Server, newConfig \in SUBSET Server, nv \in Version : Reconfig(p, newConfig, nv) BY <2>1
    ...
    <2>2. CASE SendConfigAction
        <3>1. PICK u \in Server, v \in Server, nt \in Term, nv \in Version : SendConfig(u, v, nt, nv) BY <2>2
        ...
        <3>. QED BY <3>4, <3>5, <3>6, <3>7 DEF Inv2973_1_1_def, ActiveConfigSet
    <2>3. CASE BecomeLeaderAction
        ...
        <3>3. \A s \in ActiveConfigSet' : s \in ActiveConfigSet 
          ...
          DEF ActiveConfigSet, BecomeLeader
  ...
  <1>4. Inv1758_1_0_def'
    ...
    <2>3. CASE BecomeLeaderAction
        ...
        <3>1. Inv1758_1_0_def_restated'
            ...
            <4>. PICK p \in Server, nt \in Term : \E Q \in Quorums(config[p]) : BecomeLeader(p, Q, nt) BY <2>3 DEF BecomeLeader
            <4>. CASE s = p \/ t = p 
  ...
  <1>5. Inv3915_1_1_def'
    ...
    <2>2. CASE SendConfigAction
      ...
      SendConfig,IsNewerConfig
    <2>3. CASE BecomeLeaderAction
        <3>1. PICK s \in Server : \E Q \in Quorums(config[s]) : \E nt \in Term : BecomeLeader(s, Q, nt) BY <2>3 DEF BecomeLeader
        <3>2. \A t \in Server : currentTerm[s] >= configTerm[t] BY <3>1, ElectedLeadersCurrentTermGreaterThanConfigTerms DEF IndAuto
        <3>. QED 
          BY <3>1, <3>2 
          DEF BecomeLeader, TypeOK, IndAuto,Inv3915_1_1_def,IsNewerConfig
    <2>4. CASE UpdateTermsAction
      BY <2>4 DEF TypeOK,Init,Next,IndAuto,Safety,
      Inv2973_1_1_def,Inv1758_1_0_def,Inv3915_1_1_def,Inv3246_1_2_def,Inv866_1_0_def,
    ...
    <2>1. CASE ReconfigAction
        ...
        <3>5. CASE t # p
            ...
            <4>2. PICK pQ \in Quorums(config[p]) : \A n \in pQ : currentTerm[n] = currentTerm[p]
                BY <3>p, QuorumsIdentical DEF Reconfig, QuorumsAt, ConfigIsCommitted, IndAuto, TermQuorumCheck
        ...
        <3>6. CASE t = p
            <4>1. PICK pQ \in Quorums(config[p]) : \A n \in pQ : currentTerm[n] = currentTerm[p]
                BY <3>p DEF Reconfig, ConfigIsCommitted, IndAuto, QuorumsAt, TermQuorumCheck
            <4>2. config'[t] = newConfig BY <2>ok, <3>p, <3>6 DEF Reconfig, TypeOK
            <4>3. TAKE tQ \in Quorums(config'[t])
            <4>4. QuorumsOverlap(config[p], newConfig) BY OneSymmDiffImpliesOverlap, <2>ok, <3>p, <4>1, <4>2, <4>3 DEF Reconfig, TypeOK
        ...
        <3>. QED BY <3>5, <3>6
    <2>2. CASE SendConfigAction
        <3>1. PICK u,v \in Server, nv \in Version, nt \in Term : SendConfig(u, v, nt, nv) BY <2>2
        ...
        <3>6. CASE t = v
            ...
            BY <3>1, <4>2 DEF IndAuto, Inv3246_1_2_def, ActiveConfigSet
        ...
        <3>. QED BY <3>5, <3>6
    <2>3. CASE BecomeLeaderAction
        <3>p. PICK p \in Server, newTerm \in Term : \E Q \in Quorums(config[p]) : BecomeLeader(p, Q, newTerm) BY <2>3 DEF BecomeLeader
        <3>q. PICK pQ \in Quorums(config[p]), nt2 \in Term : BecomeLeader(p, pQ, nt2) BY <3>p
        <3>1. SUFFICES ASSUME TRUE
              PROVE \A s \in Server : \A t \in ActiveConfigSet' :
                \A Q \in Quorums(config'[t]) : \E n \in Q : currentTerm'[n] >= configTerm'[s]
              BY DEF Inv3246_1_2_def, ActiveConfigSet, ConfigDisabled
        <3>2. TAKE s \in Server
        <3>3. TAKE t \in ActiveConfigSet'
        <3>4. t \in ActiveConfigSet BY <2>3, <3>3, BecomeLeaderActiveConfigSetIdentical DEF TypeOK, BecomeLeader
