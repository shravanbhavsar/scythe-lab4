...
\* This is a database manager action.gt
ResetDatabaseManager(n) ==
    /\ __ResetDatabaseManager_g1__ \* gt: reset_flag
    /\ __ResetDatabaseManager_g2__ \* gt: ~to_abort[n]
    /\ __ResetDatabaseManager_g3__ \* gt: ~to_commit[n]
    /\ frozen' = __ResetDatabaseManager_frozen__ \* gt: [frozen EXCEPT ![n] = FALSE]
    /\ aborted' = __ResetDatabaseManager_aborted__ \* gt: [abort EXCEPT ![n] = FALSE]
    /\ committed' = __ResetDatabaseManager_committed__ \* gt: [commit EXCEPT ![n] = FALSE
    /\ voted_no' = __ResetDatabaseManager_voted_no__ \* gt: [voted_no EXCEPT ![n] = FALSE]
    /\ voted_yes' =__ResetDatabaseManager_voted_yes__ \* gt: [voted_yes EXCEPT ![n] = FALSE]
    /\ UNCHANGED db_man_uncontrollable
    /\ UNCHANGED <<to_abort, to_commit, to_vote>>

\* This is a database manager action.
VoteYes(n) ==
    /\ __VoteYes_g1__ \* gt: to_vote[n]
    /\ __VoteYes_g2__ \* gt: compatible[n]
    /\ to_vote' = __VoteYes_to_vote__ \* gt: [to_vote EXCEPT ![n] = FALSE]
    /\ voted_yes' = __VoteYes_voted_yes__ \* gt: [voted_yes EXCEPT ![n] = TRUE]
    /\ frozen' = __VoteYes_frozen__ \* gt: [frozen EXCEPT ![n] = TRUE] 
    /\ UNCHANGED db_man_uncontrollable
    /\ UNCHANGED <<aborted, committed, to_abort, to_commit, voted_no>>

\* This is a database manager action.
VoteNo(n) ==
    /\ __VoteNo_g1__ \* gt: to_vote[n]
    /\ __VoteNo_g2__ \* gt: ~compatible[n]
    /\ to_vote' = __VoteNo_to_vote__ \* gt: [to_vote EXCEPT ![n] = FALSE]
    /\ voted_no' = __VoteNo_voted_no__ \* gt: [voted_no EXCEPT ![n] = TRUE]
    /\ frozen' = __VoteNo_frozen__ \* gt: [frozen EXCEPT ![n] = TRUE]
    /\ UNCHANGED db_man_uncontrollable
    /\ UNCHANGED <<aborted, committed, to_abort, to_commit, voted_yes>>

\* This is a database manager action.
Commit(n) ==
    /\ __Commit_g1__ \* gt: to_commit[n]
    /\ to_commit' = __Commit_to_commit__ \* gt: [to_commit EXCEPT ![n] = FALSE]
    /\ committed' = __Commit_committed__ \* gt: [committed EXCEPT ![n] = TRUE]
    /\ UNCHANGED db_man_uncontrollable
    /\ UNCHANGED <<aborted, frozen, to_abort, to_vote, voted_no, voted_yes>>

\* This is a database manager action.
Abort(n) ==
    /\ __Abort_g1__ \* gt: to_abort[n]
    /\ to_abort' = __Abort_to_abort__ \* gt: [to_abort EXCEPT ![n] = FALSE
    /\ aborted' = __Abort_aborted__ \* gt: [aborted EXCEPT ![n] = TRUE]
    /\ UNCHANGED db_man_uncontrollable
