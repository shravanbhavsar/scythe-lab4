...
BecomeLeader(i, voters) == 
    ...
    /\ voters \in Quorums(config[i])
    /\ i \in config[i]
    /\ i \in voters \* The new leader should vote for itself.
    /\ \A v \in voters : CanVoteForOplog(v, i, newTerm)
    /\ \A v \in voters : CanVoteForConfig(v, i, newTerm) \* TODO: could eventually be a hole.
...
CommitEntry(i, commitNodes) ==
    ...
    \* all nodes have this log entry and are in the term of the leader.
    /\ ImmediatelyCommitted(<<ind,currentTerm[i]>>, commitNodes)
...
Reconfig(i, newConfig) ==
    /\ state[i] = Primary
    \* /\ ConfigQuorumCheck(i)
    \* /\ \E Q \in {x \in SUBSET(config[i]) : Cardinality(x) * 2 > Cardinality(config[i])} : 
    \*     \A t \in Q : 
    \*         /\ configVersion[t] = configVersion[i]
    \*         /\ configTerm[t] = configTerm[i]
    /\ __u3__
    ...
    /\ UNCHANGED <<currentTerm, state, log, immediatelyCommitted, configTerm>>
...
Next == 
    ...
    \/ \E s \in Server : \E t \in Server : GetEntries(s, t)
    \/ \E s, t \in Server : RollbackEntries(s, t)
    \/ \E s \in Server, Q \in SUBSET(Server) :  BecomeLeader(s, Q)
    \/ \E s \in Server, Q \in SUBSET(Server) : CommitEntry(s, Q)
...
\* If all servers are less than MaxTerm, then primary should be electable in newer term.
PrimaryElectedInfinitelyOften == 
    []<>(\E t \in Server : 
            /\ state[t] = Primary 
            /\ \A s \in Server : currentTerm[t] > currentTerm[s])

\* ReconfigurationAlwaysPossible == 
    \* []<>(\E s \in Server, newConfig \in AllConfigs : Reconfig(s, newConfig))
