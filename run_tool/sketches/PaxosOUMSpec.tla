...
(***************************************************************************)
CONSTANTS Value, Acceptor, Quorum, ClientRequest, Ballot
...
None == CHOOSE v ∈ (Ballot ∪ {-1}) : v ∉ Ballot
  (*************************************************************************)
...
OUMSend(req) ==
  /\ seqNo < MaxSeq
  /\ seqNo'   = seqNo + 1
  /\ pending' = pending ∪ {[req ↦ req, seq ↦ seqNo']}
  /\ UNCHANGED delivered
...
CombinedNext ==
  \/ (Next      /\ UNCHANGED << seqNo, pending, delivered >>)
  \/ (NextOUM   /\ UNCHANGED << maxBal, maxVBal, maxVal, msgs >>)

CombinedSpec ==
  CombinedInit
  /\ [][CombinedNext]_AllVars
