...
Reconfig(i, newConfig, newVersion) ==
    /\ __Reconfig(i, newConfig, newVersion)_g0__ \* gt: state[i] = Primary
    /\ __Reconfig(i, newConfig, newVersion)_g1__ \* gt: ConfigQuorumCheck(i)
    /\ __Reconfig(i, newConfig, newVersion)_g2__ \* gt: TermQuorumCheck(i)
    /\ __Reconfig(i, newConfig, newVersion)_g3__ \* gt: QuorumsOverlap(config[i], newConfig)
    /\ __Reconfig(i, newConfig, newVersion)_g4__ \* gt: OplogCommitment(i)
    /\ __Reconfig(i, newConfig, newVersion)_g5__ \* gt: i \in newConfig
    /\ __Reconfig(i, newConfig, newVersion)_g6__ \* gt: newConfig # config[i]
    /\ __Reconfig(i, newConfig, newVersion)_g7__ \* gt: newVersion = configVersion[i] + 1
    /\ configTerm' = __Reconfig(i, newConfig, newVersion)_configTerm__ \* gt: [configTerm EXCEPT ![i] = currentTerm[i]]
    /\ configVersion' = __Reconfig(i, newConfig, newVersion)_configVersion__ \* gt: [configVersion EXCEPT ![i] = newVersion]
    /\ config' = __Reconfig(i, newConfig, newVersion)_config__ \* gt: [config EXCEPT ![i] = newConfig]
    /\ currentTerm' = __Reconfig(i, newConfig, newVersion)_currentTerm__ \* gt: currentTerm
    /\ state' = __Reconfig(i, newConfig, newVersion)_state__ \* gt: state
    /\ log' = __Reconfig(i, newConfig, newVersion)_log__ \* gt: log
    /\ immediatelyCommitted' = __Reconfig(i, newConfig, newVersion)_immediatelyCommitted__ \* gt: immediatelyCommitted
...
Spec == 
    ...
    /\ [][Next]_vars 
    /\ WF_vars(Next)
    \* /\ \A s2 \in Server : SF_vars(\E s1 \in Server : GetEntries(s1, s2))
    \* /\ \A s1,s2 \in Server : SF_vars(GetEntries(s1, s2))
    \* /\ \A s1,s2 \in Server : SF_vars(RollbackEntries(s1, s2))
    \* /\ \A s2 \in Server : SF_vars(\E s1 \in Server : RollbackEntries(s1, s2))
    \* /\ \A newConfig \in AllConfigs : 
    \*     SF_vars(\A s \in Server : Reconfig(s, newConfig))
...
\* Apart from RollBackEntries-GetEntries loops, the protocol should terminate.
Liveness == 
    \/ <> (termination /\ \E s \in Server : configVersion[s] # 1)
    \/ []<> \E s \in Server : Len(log[s]) < MaxLogLen
    \/ <> (immediatelyCommitted # {})
\* </properties>
